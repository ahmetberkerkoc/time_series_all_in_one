    ...
    for m3_type in ["monthly", "quarterly", "yearly"]:
        ...
        try:
            ...
            for i in csv_list:
                ...
                y_normalized = y_scaler.fit_transform(y.to_frame()).ravel()
                y_normalized = pd.Series(y_normalized, index=y.index, name="y")
                X = pd.DataFrame(MinMaxScaler().fit_transform(X), index=X.index, columns=X.columns)

                pf = filter_sgbdtsx(y_normalized, sx_order=order, exog=X, n_particles=len(y_normalized))
                stats_sx = SARIMAX(y_normalized, X,order=order).fit()
                ...
                ax = y_normalized.iloc[-test_len:].plot(label="true (normalized)")
                ...
                test_mse = mse(y_pred=our_preds[-test_len:], y_true=y_normalized[-test_len:])
                test_mae = mae(y_pred=our_preds[-test_len:], y_true=y_normalized[-test_len:])
                test_mape = mape(y_pred=our_preds[-test_len:], y_true=y_normalized[-test_len:])
                score_dict["our"] = (test_mse, test_mae, test_mape)

                sarimax_test_mse = mse(y_pred=stats_preds[-test_len:], y_true=y_normalized[-test_len:])
                sarimax_test_mae = mae(y_pred=stats_preds[-test_len:], y_true=y_normalized[-test_len:])
                sarimax_test_mape = mape(y_pred=stats_preds[-test_len:], y_true=y_normalized[-test_len:])
                ...
                y_train, y_test = y_normalized.iloc[:-forecast_horizion], y_normalized.iloc[-forecast_horizion:]
                ...
                # LGBM
                regressor = lgb.LGBMRegressor(random_state=0)
                regressor.fit(X_train, y_train)
                model_pred = regressor.predict(X_test)
                model_pred_series = pd.Series(model_pred,index=range(train_len,train_len + forecast_horizion))
                ...
                # NAIVE
                model_pred = y_normalized[-test_len - 1 : -1]
                model_pred_series = pd.Series(model_pred,index=range(train_len,train_len + forecast_horizion))
                ...
                pd.to_pickle(ALL_RESULT_DICT,f"ALL_RESULT_m3_{m3_type}_normalized.pkl")
