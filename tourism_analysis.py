...
def second_method(
    ...
    metric_to_index = {"MSE": 0, "MAE": 1, "MAPE": 2, "MASE": 3}
    metric = metric.upper()
...
if __name__ == "__main__":
    #first_method()
    tourism_data_name = "tourism"
    data = pd.read_pickle("ALL_RESULT_tourism.pkl")
    # data = {
    #     key: value for key, value in data.items() if key.startswith(f"{tourism_data_name}_")
    # }
    
    print(f"Running second_method for {tourism_data_name} data with {len(data)} csv files.")
    try:
        second_method(sample_size=1, max_iterations=100000, data=data, data_type=tourism_data_name, metric="MASE")
