...
# iTransformer inverted Transformer model.
from neuralforecast.models import iTransformer
# Metric used to score forecasts.
from sklearn.metrics import mean_absolute_error
# Loss used to train iTransformer.
...
# iTransformer version without exogenous variables.
# This keeps the comparison clean for NeuralForecast versions where iTransformer
# raises errors for future or historical exogenous variables. Lag features are
# still used through the target lookback window controlled by input_size.

# Default configuration used when tuning is skipped.
DEFAULT_PARAMS = {
    # Use the last 7 real target values: y[t-7], ..., y[t-1].
    # Therefore lag-1 is included in the iTransformer autoregressive input.
    "input_size": 7,
    "hidden_size": 128,
    "n_heads": 2,
    "e_layers": 2,
...
def to_neuralforecast_format(df: pd.DataFrame) -> pd.DataFrame:
    """Convert the dataframe to NeuralForecast format for iTransformer.

    No exogenous columns are attached here. The model receives only:
    - unique_id
    - ds
    - y

    With input_size=7, iTransformer still uses the real target history
    y[t-7], ..., y[t-1], so lag-1 is included.
    """
    return pd.DataFrame(
        {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use-saved-params",
        action="store_true",
        help="Load parameters from itransformer_no_exog_best_params.txt if it exists.",
    )
    parser.add_argument(
        "--tune-limit",
        type=int,
...
def build_model(train_nf: pd.DataFrame, model_params: dict) -> iTransformer:
    """Create a one-step-ahead iTransformer model configured from chosen parameters."""
    return iTransformer(
        h=1,
        input_size=max(1, min(model_params["input_size"], len(train_nf) // 2)),
        # There is one target time series: unique_id == "delhi_climate".
        n_series=train_nf["unique_id"].nunique(),
        # Do not pass futr_exog_list or hist_exog_list.
        # This version uses only the autoregressive target history.
        exclude_insample_y=False,  # keep autoregressive y history; lag-1 is used
        hidden_size=model_params["hidden_size"],
        n_heads=model_params["n_heads"],
        e_layers=model_params["e_layers"],
        d_layers=model_params["d_layers"],
        d_ff=model_params["d_ff"],
        factor=model_params["factor"],
        dropout=model_params["dropout"],
        use_norm=model_params["use_norm"],
...
def extract_point_forecast(forecast_df: pd.DataFrame) -> float:
    """Extract a scalar point forecast from iTransformer output."""
    if "iTransformer" not in forecast_df.columns:
        raise ValueError(
            f"Could not find an iTransformer prediction column. "
            f"Columns: {forecast_df.columns.tolist()}"
        )
    return float(forecast_df["iTransformer"].iloc[0])
...
def walk_forward_one_step_predict(
    ...
    prediction. With input_size=7, iTransformer receives the last 7 real target values
    in its autoregressive window whenever they are available. No exogenous
    variables are used in this version.
    """
    ...
    for step, (_, row) in enumerate(test_df.iterrows()):
        ...
        # No futr_df is passed because this iTransformer version does not use
        # future or historical exogenous variables. The forecast uses only the
        # observed target window in history_nf.
...
def main() -> None:
    ...
    predictions_path = Path("DailyDelhiClimate_itransformer_no_exog_predictions.csv")
    params_path = Path("itransformer_no_exog_best_params.txt")
